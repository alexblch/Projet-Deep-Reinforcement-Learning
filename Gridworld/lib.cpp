...
public:
    ...
    std::tuple<std::vector<int>, std::vector<double>> run() {
        ...
        while (true) {
            ...
            // Si la politique est stable, on peut arrêter
            if (policy_stable) {
                break;
            }
        }
    ...
    void policy_evaluation(std::vector<int>& policy, std::vector<double>& V) {
        while (true) {
            ...
            for (int s = 0; s < num_states; ++s) {
                double v = 0;
                // Évaluation de la valeur de l'état en suivant la politique actuelle
                for (int next_state = 0; next_state < num_states; ++next_state) {
                    for (int r_index = 0; r_index < env.attr("num_rewards")().cast<int>(); ++r_index) {
                        double prob = env.attr("p")(s, policy[s], next_state, r_index).cast<double>();
            ...
            // Arrêter si le changement est inférieur au seuil theta
            if (delta < theta) {
                break;
            }
        }
...
class SARSA {
public:
    SARSA(py::object env, int episodes, double alpha = 0.1, double gamma = 0.99, double epsilon = 1.0, double epsilon_decay = 0.999, double min_epsilon = 0.01)
